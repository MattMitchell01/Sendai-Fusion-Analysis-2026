...
if Options.GrabTotalIntensityOnly ~= 'y'
    ...
    if strcmp(Options.DeterminePixelOffset, 'y')
        ...
        % If FFT method is prefered, perform the calculation. Try each
        % region in Options.FFTRegionList in order (middle region first,
        % then corner backups) until one is approved or all are rejected.
        if strcmp(Options.FFTOffset, 'y')
            NumRegions = numel(Options.FFTRegionList);
            FFTApproved = false;
            fftFig = figure;

            for regionIdx = 1:NumRegions
                CurrRegion = Options.FFTRegionList(regionIdx);

                disp(' ')
                fprintf('Trying FFT region %d/%d: %s (X=[%.2f %.2f], Y=[%.2f %.2f])\n', ...
                    regionIdx, NumRegions, CurrRegion.Label, ...
                    CurrRegion.X(1), CurrRegion.X(2), CurrRegion.Y(1), CurrRegion.Y(2));

                [Offset] = Fast_Fourier_Transform(VideoMatrix, ImageWidth, ImageHeight, NumFrames, FindingImage, CurrRegion);
                ...
                xlabel('Frame');
                ylabel('Pixels');
                title(sprintf('XY Drift Over Time -- Region %d/%d: %s', regionIdx, NumRegions, CurrRegion.Label));
                ...
                if strcmpi(strtrim(userResp),'y')
                    ...
                    FFTApproved = true;
                    break
                elseif regionIdx < NumRegions
                    disp(' ')
                    disp('Looks like that region did not work. Trying a backup region...')
                end
            end

            if ~FFTApproved
                disp(' ')
                disp('All FFT regions were rejected.')
        ...
        % If FFT failed or we want to do SPT instead... Keep re-tracking
        % (with a newly selected particle each time) until the user
        % approves the resulting XY shift.
        if strcmp(Options.SingleParticleTrackingOffset,'y')
            SPTApproved = false;
            sptFig = [];   % not created yet -- see note below

            while ~SPTApproved
                % Deliberately do NOT create/raise the plot figure before
                % this call. Single_Particle_Tracking needs FigureHandles.ImageWindow
                % to have real click focus for ginput; a brand-new figure
                % popping up immediately beforehand competes for that
                % focus and can eat the click silently (observed as
                % Figure 6 appearing and clicks doing nothing). The plot
                % figure is only created/raised AFTER ginput/tracking is
                % already done, matching how this worked before retries
                % were added.
                [Offset] = Single_Particle_Tracking(VideoMatrix,BWVideoMatrix,...
                    FigureHandles,Options);

                if isempty(sptFig) || ~isvalid(sptFig)
                    sptFig = figure;
                else
                    set(0,'CurrentFigure', sptFig);
                end
                clf;
                plot(Offset.x, 'b');
                hold on;
                ...
                if strcmpi(strtrim(userResp),'y')
                    disp(' ')
                    disp('Single-particle-tracking XY shifting approved. Proceeding...')
                    SPTApproved = true;
                else
                    disp(' ')
                    disp('Looks like that particle did not track well.')
                    disp('Please select another particle and try again.')
                end
            end
        end
...
elseif Options.GrabTotalIntensityOnly == 'y'
    ...
    OtherDataToSave.GlobalTimeZeroFrame      = Options.TimeZeroFrameNumber;
    OtherDataToSave.GlobalFindingImageFrame  = Options.FrameNumToFindParticles;
    OtherDataToSave.GlobalStartAnalysisFrame = Options.StartAnalysisFrameNumber;
    OtherDataToSave.FindingImage             = FindingImage;
    OtherDataToSave.GlobalFocusFrameNumbers  = Options.FocusFrameNumbers;
    if exist('Offset','var')
        OtherDataToSave.XYOffsets = Offset;
    else
        OtherDataToSave.XYOffsets = [];
    end
