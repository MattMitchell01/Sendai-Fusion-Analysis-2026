function [Offsets] = Fast_Fourier_Transform(VideoMatrix, ImageWidth, ImageHeight, NumFrames, FindingImage, Region)
...
%       - the reference image (FindingImage -- the same image particle
%         boxes were located on, at global frame Options.FrameNumToFindParticles)
%       - the current image
%
...
% OUTPUT
% ------
% Offsets.x and Offsets.y contain the X and Y shifts of each VideoMatrix
% frame relative to FindingImage (NOT relative to the previous frame, and
% NOT relative to VideoMatrix frame 1). The first entry is therefore the
% shift from the finding frame to the first analyzed (start) frame -- it
% is generally NOT [0, 0], since the finding frame and the start frame
% are not required to be the same frame.
%
...
%====================================================================================
    ...
    % ROI defined as independent X/Y percentages of image size (Region.X,
    % Region.Y -- one element of Options.FFTRegionList)
    Region_Left_X  = round(ImageWidth  * Region.X(1));
    Region_Right_X = round(ImageWidth  * Region.X(2));
    Region_Top_Y   = round(ImageHeight * Region.Y(1));
    Region_BOT_Y   = round(ImageHeight * Region.Y(2));

    % Extract reference ROI from FindingImage (the image particle boxes were located on)
    Ref_ROI = double(FindingImage( ...
