...
% ==============Options you'll probably change regularly==================
    ...
    Options.Threshold = 115; %Rem: Should not change much but dye [ ] in virus can be very different even for == samples
...
%================= Options You Might Change While Optimizing For a Particular Data Type (after consulting Bob) ================
        ...
        % y or n. Do we need to account for XY Sample-Wide Drift
            ...
            % y or n. Uses Fast-Fourier-Transform to determine pixel offset over time.
                ...
                % Ordered list of ROIs to try for FFT cross-correlation. If
                % the middle region is rejected interactively, the corner
                % regions are tried in order before falling back to
                % Single_Particle_Tracking. Percentages are of image size.
                Options.FFTRegionList(1) = struct('Label','Middle (default)',    'X',[0.30 0.70], 'Y',[0.30 0.70]);
                Options.FFTRegionList(2) = struct('Label','Top-left corner',     'X',[0.10 0.40], 'Y',[0.10 0.40]);
                Options.FFTRegionList(3) = struct('Label','Top-right corner',    'X',[0.60 0.90], 'Y',[0.10 0.40]);
                Options.FFTRegionList(4) = struct('Label','Bottom-left corner',  'X',[0.10 0.40], 'Y',[0.60 0.90]);
                Options.FFTRegionList(5) = struct('Label','Bottom-right corner', 'X',[0.60 0.90], 'Y',[0.60 0.90]);
    ...
    % ---------Inputs specific to each data set--------   
        ...
        Options.ExtractInputsFromTextFile = 'n';
            ...
            % in TextFile_Extract_Analysis_Inputs.m for the proper formatting of the text file.
